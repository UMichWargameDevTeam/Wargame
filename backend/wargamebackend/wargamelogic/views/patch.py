...
@require_any_role_instance([
    ...
    },
    {
        ...
        "role.branch": lambda request, kwargs: get_object_or_404(UnitInstance, pk=kwargs["pk"]).unit.branches.all(),
        "role.is_operations": lambda request, kwargs: not get_object_or_404(UnitInstance, pk=kwargs["pk"]).unit.is_logistic,
        "role.is_logistics": lambda request, kwargs: get_object_or_404(UnitInstance, pk=kwargs["pk"]).unit.is_logistic 
    }
])
def move_unit_instance(request, pk, row, column):
    unit_instance = get_object_or_404(UnitInstance, pk=pk)
...
def use_attack(request, pk, attack_name):
    ...
    role_instance = get_object_or_404(
        RoleInstance,
        user=request.user,
        team_instance=unit_instance.team_instance
    )
