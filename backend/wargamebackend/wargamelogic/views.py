...
@api_view(['GET'])
def get_attacks_for_unit_by_name(request, unit_name):
    unit = get_object_or_404(Unit, name=unit_name)
    attacks = get_list_or_404(Attack, unit=unit)
    serializer = AttackSerializer(attacks, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def get_abilities_for_unit_by_name(request, unit_name):
    unit = get_object_or_404(Unit, name=unit_name)
    abilities = get_list_or_404(Ability, unit=unit)
